    ...
    <div class="shop-sidebar">
        <div class="menu-nav">
            ...
            <div class="collapse multi-collapse show" id="multiCollapseExample1">
                <div class="item-check">
                    @if($post_type == 'buy')
                    <a href="{{ url('hang-hoa-xnk/hang-nhap-khau') }}" class="item-radius" title="{{ $category->slug }}">Tất cả</a>
                    @else
                ...
                @foreach($categories as $item)
                    ...
                    @endphp
                <div class="item-check">
                    <a href="{{ $item->getUrl($category->slug??'', $post_type) }}" class="item-radius{{ $menu_active }}" title="{{ $category->slug }}">{{ $item->name }}</a>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="menu-nav">
            ...
            <div class="collapse multi-collapse show" id="multiCollapseExample3">
                ...
                @if($index<8)
                <div class="item-check">
                    <a href="{{ request()->fullUrlWithQuery(['country' => $item->name, 'destination' => request('destination')]) }}" class="item-radius">{{ $item->name_vi!= '' ? $item->name_vi :$item->name }}</a>
                </div>
                @endif
                @endforeach
            </div>
