                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-sm-5 col-8">
                            <select name="category_id" class="form-select">
                                ...
                                @foreach($categoriesTitle as $category_id => $item)
                                <option value="{{ $category_id }}" {{ request('category_id')==$category_id?'selected' : '' }}>{{ $item }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-sm-5 col-8">
            ...
            <table class="table table-bordered" id="table_index">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin_client.edit', $item->id) }}" title=""><b>{{ $item->name }}</b></a>
                        </td>
                        <td>
                            @php
                            ...
                            @foreach($categories as $index => $category)
                                @if($index > 0), @endif
                                <a href="{{ route('admin_post.category_edit', $category->id) }}" target="_blank">{{ $categoriesTitle[$category->id] }}</a>
                            @endforeach
                        </td>
                        <td class="text-center">
                            @if($item->image)
