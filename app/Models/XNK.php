...
class XNK extends Model
{
    protected $table = 'xuatnhapkhau';
    protected $guarded = [];
    ...
    {
        return $this->hasMany(XNKDescription::class, 'post_id', 'id');
    }
    ...
    public function categories(){
        return $this->belongsToMany(NewsCategory::class, 'post_category_join', 'post_id', 'category_id');
    }
    ...
    public function getList(array $dataSearch)
    {
        ...
        $tableDescription = (new \App\Models\PostDescription)->getTable();
        $tablePTC         = (new \App\Models\PostCategoryJoin)->getTable();
        $tableProduct     = (new News)->getTable();

        //Select field
        $dataSelect = $tableProduct.'.*, '.$tableDescription.'.name, '.$tableDescription.'.description, '.$tableDescription.'.content';

        $productList = (new News)
