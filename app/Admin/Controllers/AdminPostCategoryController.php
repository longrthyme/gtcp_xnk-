...
use App\Admin\Models\AdminPostCategory;
...
class AdminPostCategoryController extends Controller
{
    ...
    public $admin_view = 'admin.post-category';
    public $languages;
    /**
    ...
    public function index(){
        ...
        $posts = (new AdminPostCategory)->getCategoryListAdmin($dataSearch);
        $data = [
            'title'            => $this->title_head,
            'languages'            => $this->languages,
            'posts'            => $posts,
            'url_create'            => route('admin_post.category_create'),
            'urlDeleteItem'            => route('admin_post.category_delete'),
        ];
        return view($this->admin_view .'.index', $data);
    }

    public function create(){
        $categories = (new AdminPostCategory)->getTreeCategoriesAdmin();
        $data = [
            'title'            => $this->title_head,
            'categories'            => $categories,
            'languages'            => $this->languages,
            'url_post'            => route('admin_post.category_post'),
        ];
    ...
    public function edit($id){
        $categories = (new AdminPostCategory)->getTreeCategoriesAdmin();
        $post = AdminPostCategory::find($id);
        if($post){
            $data = [
                'title'            => $this->title_head,
                'categories'            => $categories,
    ...
    public function post(){
        ...
        if($post_id)
        {
            $category = AdminPostCategory::find($post_id); 
            $category->update($data_db);
            $category->descriptions()->delete();
        }
        else
        {
            $respons = AdminPostCategory::create($data_db);
            $post_id = $respons->id;
        }
        ...
        foreach ($data['description'] as $code => $row) 
        {
            ...
            AdminPostCategory::insertDescription($dataDes);
        }
        //description
        if($data['image'] != '')
        {
            $ext = pathinfo(public_path($data['image']), PATHINFO_EXTENSION);
            if(in_array($ext, ['xlsx']))
            {
                ini_set('memory_limit', '2048M');
                ini_set('max_execution_time', '300');

                // (new \App\Tasks\ExcelTask)->getData($data['image']);
                $importJob = new \App\Jobs\ProcessImportExcel( $data['image']);
        ...
        if($save=='apply')
        {
            ...
            $url = route('admin_post.category_edit', array($post_id));
            Helpers::msg_move_page($msg, $url);
        }
        else
            return redirect(route('admin_post.category'));
    }
    ...
    public function deleteList()
    {
        if (!request()->ajax()) {
            ...
            AdminPostCategory::destroy($arrID);
    ...
    {
        return AdminPostCategory::find($id);
    }
}
