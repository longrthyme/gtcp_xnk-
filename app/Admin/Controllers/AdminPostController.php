...
use App\Admin\Models\AdminPost, App\Admin\Models\AdminPostCategory;

class AdminPostController extends RootAdminController
{
    public $title_head = 'Tin tức';
    public $admin_view = 'admin.post';
    public $languages;
    /**
    ...
    public function index(){
        $categoriesTitle = AdminPostCategory::getListTitleAdmin();
        ...
        $posts = (new AdminPost)->getListAdmin($dataSearch);
        ...
        $data = [
            ...
            'url_create'            => route('admin_post.create'),
            'urlDeleteItem'            => route('admin_post.delete'),
        ];
    ...
    public function create(){
        ...
        $data = [
            ...
            'url_post'            => route('admin_post.post'),
        ];
    ...
    public function post(){
        ...
        if($post_id == 0){
            $post = AdminPost::create($data_db);
            $post_id= $post->id;
        } else{
            $post = AdminPost::find($post_id);
            $post->update($data_db);
        ...
        foreach ($data['description'] as $code => $row) 
        {
            ...
            AdminPost::insertDescription($dataDes);
        }        
        //description

        if($save == 'apply'){
            $msg = "Post has been Updated";
            $url = route('admin_post.edit', array($post_id));
