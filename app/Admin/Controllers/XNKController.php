...
use App\Admin\Models\AdminXNK, App\Admin\Models\AdminXNKCategory;

class XNKController extends Controller
{
    public $title_head = 'Xuất nhập khẩu';
    public $admin_view = 'admin.xnk';
    public $languages;
    /**
    ...
    public function index(){
        $categoriesTitle = AdminXNKCategory::getListTitleAdmin();
        ...
        $posts = (new AdminXNK)->getListAdmin($dataSearch);
        ...
        $data = [
            ...
            'url_create'            => route('admin_xnk.create'),
            'urlDeleteItem'            => route('admin_xnk.delete'),
        ];
    ...
    public function create(){
        $categories = (new AdminXNKCategory)->getTreeCategoriesAdmin();
        // dd($categories);
    ...
    public function post(){
        ...
        if($post_id == 0){
            $post = AdminXNK::create($data_db);
            $post_id= $post->id;
        } else{
            $post = AdminXNK::find($post_id);
            $post->update($data_db);
        ...
        foreach ($data['description'] as $code => $row) 
        {
            ...
            Post::insertDescription($dataDes);
        }        
        //description

        if($save == 'apply'){
            $msg = "Post has been Updated";
            $url = route('admin_xnk.edit', array($post_id));
            Helpers::msg_move_page($msg, $url);
        }
        else{
            return redirect(route('admin_xnk'));
        }
    }
