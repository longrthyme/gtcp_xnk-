...
use App\Admin\Models\AdminLogistic, App\Admin\Models\AdminLogisticCategory;

class LogisticController extends Controller
{
    public $title_head = 'Xuất nhập khẩu';
    public $admin_view = 'admin.logistic';
    public $languages;
    /**
    ...
    public function index(){
        $categoriesTitle = AdminLogisticCategory::getListTitleAdmin();
        ...
        $posts = (new AdminLogistic)->getListAdmin($dataSearch);
        ...
        $data = [
            ...
            'url_create'            => route('admin_logistic.create'),
            'urlDeleteItem'            => route('admin_logistic.delete'),
        ];
    ...
    public function create(){
        $categories = (new AdminLogisticCategory)->getTreeCategoriesAdmin();
        // dd($categories);
        ...
        $data = [
            ...
            'url_post'            => route('admin_logistic.post'),
        ];
    ...
    public function edit($id){
        $categories = (new AdminLogisticCategory)->getTreeCategoriesAdmin();
        $post = AdminLogistic::findorfail($id);
        $data = [
            'title'            => $this->title_head,
            'languages'            => $this->languages,
            'post'            => $post,
    ...
    public function post(){
        ...
        if($post_id == 0){
            $post = AdminLogistic::create($data_db);
            $post_id= $post->id;
        } else{
            $post = AdminLogistic::find($post_id);
            $post->update($data_db);
        ...
        foreach ($data['description'] as $code => $row) 
        {
            ...
            AdminLogistic::insertDescription($dataDes);
        }        
        //description

        if($save == 'apply'){
            $msg = "Post has been Updated";
            $url = route('admin_logistic.edit', array($post_id));
            Helpers::msg_move_page($msg, $url);
        }
        else{
            return redirect(route('admin_logistic'));
        }
    }
