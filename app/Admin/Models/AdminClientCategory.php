...
use App\Models\ClientCategoryDescription;

class AdminClientCategory extends ClientCategory
{
    ...
    public static function getCategoryListAdmin(array $dataSearch)
    {
        ...
        $tableCategory     = (new ClientCategory)->getTable();

        $categoryList = (new ClientCategory)
            ->leftJoin($tableDescription, $tableDescription . '.category_id', $tableCategory . '.id')
            ->where($tableDescription . '.lang', sc_get_locale());
        if ($keyword) {
            $categoryList = $categoryList->where(function ($sql) use ($tableDescription, $keyword, $tableCategory) {
                $sql->where($tableDescription . '.name', 'like', '%' . $keyword . '%')->orwhere($tableCategory . '.id', $keyword);
            });
        }
    ...
    {
        $tableDescription = (new ClientCategoryDescription)->getTable();
        $table = (new ClientCategory)->getTable();
